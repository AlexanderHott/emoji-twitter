...
export const postsRouter = createTRPCRouter({
  ...
  reposts: publicProcedure
    .input(z.object({ userId: z.string() }))
    .query(async ({ ctx, input }) => {
      return await ctx.prisma.repost.findMany({
        where: { userId: input.userId },
        orderBy: { createdAt: "desc" },
        take: 100,
        include: {
  ...
  postsAndReposts: publicProcedure
    .input(z.object({ userId: z.string() }))
    .query(async ({ ctx, input: { userId } }) => {
      const postsAndReposts = await ctx.prisma.post.findMany({
        where: {
          OR: [
            {
        ...
        include: {
          reposts: {
            orderBy: {
              createdAt: "desc", // Order by repost creation date in descending order
            },
          },
          userLikes: true, // Include user likes information
          _count: { select: { userLikes: true } },
        },
        orderBy: {
      ...
      return addUserDataToPosts(postsAndReposts);
